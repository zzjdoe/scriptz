...
echo -e "${LightPurple}AirCrack Send De-Authentication Helper Utility v1.0.0-alpha.1${NC}"
...
if [[ "$2" == "" ]]
  then
    echo -e "[${Red}-${NC}] ${LightRed}ERROR: Usage is: $0 NIC MAC SSID${NC}"
    echo -e "    e.g.: $0 wlan0 0A:D4:0C:58:B1:B3"
    exit 2
fi
